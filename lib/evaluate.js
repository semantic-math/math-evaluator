 ...
 * evaluate
 */
import {gcd, lcm, nthRoot} from './functions'
...
const operations = {
    ...
    'nthRoot': ([radicand, index]) => nthRoot(radicand, index),
}

const functions = {gcd, lcm}
    ...
    switch (node.type) {
        ...
        case 'Apply': {
            ...
            if (op in operations) {
                ...
                if (fn in functions) {
                    return functions[fn](...args.map(evaluate))
                } else if (fn in Math) {
                    if (Math[fn].length === args.length) {
                        return Math[fn](...args.map(evaluate))
                    } else {
                        throw new Error(`'${fn}' takes ${Math[fn].length} parameters, ${args.length} was provided`);
                    }
                } else {
                    throw new Error(`'${op}' is not a valid function in this context`);
                }
            } else {
